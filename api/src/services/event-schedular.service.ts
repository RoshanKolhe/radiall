...
import { ApprovalUsersRepository, InternalValidationFormRepository, MaintainancePlanRepository, ToolsRepository } from "../repositories";
...
export class EventSchedular {
    ...
    async createValidationHistory(validationFormId: number) {
        try {
            ...
            const toolData = await this.toolsRepository.findById(validationForm?.toolsId);
            if (!toolData) {
                throw new HttpErrors.NotFound('No tool found');
            }

            if(toolData?.isMaintaincePlanNeeded){
                const maintainancePlan = await this.maintainancePlanRepository.findById(toolData?.levelTwoMaintainanceId);

                if(!maintainancePlan){
                    throw new HttpErrors.NotFound('No level 2 maintainance plan found');
                }
                ...
                const nextYearDate = new Date(validationForm.functionalTestingQuestionery?.date);
                ...
                nextYearDate.setHours(nextYearDate.getHours() + (maintainancePlan?.periodicity*24));
                ...
                await this.toolsRepository.updateById(validationForm?.toolsId, {revalidationDate : nextYearDate});
            }
        } catch (error) {
